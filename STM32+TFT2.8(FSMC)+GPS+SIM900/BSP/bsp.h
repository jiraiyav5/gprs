...
#define  BSP_GLED_ON()    GPIO_SetBits(  GPIOD, GPIO_Pin_13)  
#define  BSP_GLED_OFF()   GPIO_ResetBits(GPIOD, GPIO_Pin_13)

#define  BSP_RLED_ON()    GPIO_SetBits(  GPIOD, GPIO_Pin_12)  
#define  BSP_RLED_OFF()   GPIO_ResetBits(GPIOD, GPIO_Pin_12)

#define  BSP_BEEP_ON()    GPIO_SetBits(  GPIOB, GPIO_Pin_9)  
#define  BSP_BEEP_OFF()   GPIO_ResetBits(GPIOB, GPIO_Pin_9)

#define  GSM_POWER_KEY_LOW()     GPIO_SetBits(  GPIOA, GPIO_Pin_8)  
#define  GSM_POWER_KEY_HIGH()    GPIO_ResetBits(GPIOA, GPIO_Pin_8)
...
#define Key_Up()       GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_2)
#define Key_Down()	   GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_1)
#define Key_Left()	   GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_0)
//输入宏定义 GSM 模块状态脚
#define GSM_ON1_OFF0() GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_6)



#define ADC1_DR_Address 	((INT32U)0x4001244C)
#define ADC_RCVTab_SIZE 	1		 //用于ADC转换的通道数
#define ADC_CONV_COUNT 		10		 //转换次数计数器


#define ADC_TEMP	0


BSP_EXT OS_EVENT       		*App_ADCSem;
BSP_EXT DMA_InitTypeDef 	DMA_InitStructure;
BSP_EXT ADC_InitTypeDef		ADC_InitStructure;
BSP_EXT INT16U 				ADC_RCVTab    [ADC_RCVTab_SIZE];
BSP_EXT INT32S 				ADC_ValueMux  [ADC_RCVTab_SIZE];
BSP_EXT INT32S        	    ADC_Result    [ADC_RCVTab_SIZE];//ADC最终结果
BSP_EXT INT8U				ADC_CurConvCount;
BSP_EXT INT8U	            ADC_Complete;	  //转换结束标志
/*
*********************************************************************************************************
*                                               MODULE END
*********************************************************************************************************
